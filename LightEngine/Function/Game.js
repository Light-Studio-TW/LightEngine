...
class CREATE {
  ...
  async audio (data) {
    ...
    if (games[this.id] === undefined) {
      ...
      return new AUDIO(this.id, data.id)
    }
  }
  //創建物件
  object (data) {
...
class GAME_SDL {
  ...
  //顯示運算
  display_operations () {
    games[this.id].display = (display_operations(this.id, games[this.id].window.width, games[this.id].window.height).concat(games[this.id].draw)).sort((prevent, current) => {
...
class GAME_CANVAS {
  ...
  //顯示運算
  display_operations () {
    games[this.id].display = (display_operations(this.id, games[this.id].width, games[this.id].height).concat(games[this.id].draw)).sort((prevent, current) => {
      return prevent.layer - current.layer
    })
  }
  //聆聽事件
...
//顯示運算
function display_operations (id, width, height) {
  let display_save = []
...
module.exports = { GAME_SDL, GAME_CANVAS, createGameData, display_operations }
