...
class WINDOW {
  ...
  maximize () {
    games[this.id].change = true
    games[this.id].window.maximize(boolean)
    this.synchronized_value()
  }
  //最小化
  minimize () {
  ...
  display () {
    if (games[this.id].change) {
      ...
      games[this.id].canvas = getCanvas(this.id, games[this.id].display, games[this.id].window.width, games[this.id].window.height)
    }
...
const { display_operations } = require('./Game')
